...
export default class WorldScene extends Phaser.Scene {
  ...
  private day = 1; private season: Season = RETIRO_CONFIG.defaultSeason; private tutorialStep = TutorialStep.TALK_TO_FOREMAN; private playerPosition: { x: number; y: number; facing: PlayerDirection } = { ...RETIRO_CONFIG.playerSpawn }; private showIntro = true; private transitioning = false; private pendingTrainEntry = false;
  ...
  init(data: unknown): void {
    ...
    this.playerPosition = init.kind === 'newGame'
      ...
      : { ...init.player, facing: (init.kind === 'returnedFromAffOffice' || init.kind === 'returnedFromTrain') ? init.player.facing : RETIRO_CONFIG.playerSpawn.facing };
    this.showIntro = init.kind !== 'returnedFromAffOffice' && init.kind !== 'returnedFromTrain'; this.interactions.clear(); this.transitioning = false;
  }
  create(): void {
  ...
  private interact(): void { if (this.dialoguePanel?.isOpen) { if (this.dialoguePanel.advance() === 'closed') { if (this.pendingTrainEntry) { this.enterTrain(); this.pendingTrainEntry = false; } else { this.hud?.setObjective(getRetiroObjective(this.tutorialStep)); } } return; } const target = this.interactions.currentTarget; if (!target) return; if (target.id === 'affBoard') { this.enterOffice(); return; } if (target.id === 'trainDoor' && this.tutorialStep === TutorialStep.RETURN_TO_TRAIN) { this.pendingTrainEntry = true; const result = transitionRetiroTutorial(this.tutorialStep, target.id); this.tutorialStep = result.step; this.dialoguePanel?.open(result.dialogue); return; } const result = transitionRetiroTutorial(this.tutorialStep, target.id); this.tutorialStep = result.step; this.dialoguePanel?.open(result.dialogue); }
  ...
  private enterTrain(): void { this.transitioning = true; this.player.freeze(); this.hud?.hideInteractionPrompt(); this.cameras.main.fadeOut(220, 0, 0, 0); this.cameras.main.once(Phaser.Cameras.Scene2D.Events.FADE_OUT_COMPLETE, () => { this.scene.start('TrainInteriorScene', { kind: 'enterTrain', day: this.day, season: this.season, tutorialStep: this.tutorialStep, returnPosition: { x: RETIRO_CONFIG.trainCoach.x + RETIRO_CONFIG.trainCoach.door.localX, y: RETIRO_CONFIG.trainCoach.y + RETIRO_CONFIG.trainCoach.door.localY, facing: 'down' } }); this.cameras.main.resetFX(); }); }
